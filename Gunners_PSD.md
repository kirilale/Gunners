...
const arsenalExceptions = [
  'arsenal', 'gunners', 'cannon', 'gooner',
  'arteta', 'emirates', 'highbury', 'invincibles',
  'gunner', 'coyg' // Come On You Gunners
];
...
**Badge Type Implementation Status:**

**✅ Fully Implemented (Generated + UI Display):**
- DERBY, EUROPEAN, CUP_FINAL, CLEAN_SHEET, HIGH_SCORING, VICTORY, STANDARD

**🔮 Future Badge Types (UI Ready, Not Yet Generated):**
- COMEBACK, TROPHY

**Badge Display Configuration:**
All badge types generated by the system now have proper UI display configurations in `badge-gallery.tsx`:
```typescript
const BADGE_TYPE_DISPLAY = {
  ...
  COMEBACK: { icon: "💪", label: "Comeback", color: "bg-purple-100" }, // Future
  TROPHY: { icon: "🏆", label: "Trophy", color: "bg-yellow-100" }, // Future
};
```
...
const totalWithUser = arsenalWinsWhenPresent + arsenalDrawsWhenPresent + arsenalLossesWhenPresent;

// Minimum threshold to prevent misleading statistics from small sample sizes
const MINIMUM_MATCHES_FOR_LUCKY_CHARM = 10;

const luckyCharmPercentage = 
  totalWithUser >= MINIMUM_MATCHES_FOR_LUCKY_CHARM
    ? (arsenalWinsWhenPresent / totalWithUser) * 100
    : null; // Returns null if threshold not met

const overallWinPercentage = 
  ((arsenalWinsWhenPresent + arsenalWinsWhenAbsent) / 
   (total_matches)) * 100;

const isLuckyCharm = luckyCharmPercentage !== null && luckyCharmPercentage > overallWinPercentage;
```

**Display (when threshold met):**
```
🍀 Lucky Charm Status: 67.0%
...
**Display (when threshold not met):**
```
🍀 Lucky Charm Status: 🔒
